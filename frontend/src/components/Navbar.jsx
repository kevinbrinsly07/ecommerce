...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-slate-950 backdrop-blur-xl border-b border-slate-900/40 py-5">
      <nav className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          <div className="flex items-center gap-8">
            <Link to="/" className="text-2xl font-extrabold bg-linear-to-r from-cyan-300 via-sky-400 to-indigo-400 bg-clip-text text-transparent drop-shadow-[0_6px_25px_rgba(34,211,238,0.35)]">
              Ebucket
            </Link>
              ...
              <div className="relative">
                ...
                {isProfileOpen && (
                  <div className="absolute right-0 mt-2 w-44 rounded-xl bg-slate-950/90 backdrop-blur-md shadow-lg ring-1 ring-slate-700/50 py-1">
                    <Link to="/orders" onClick={() => setIsProfileOpen(false)} className="block px-4 py-2 text-sm font-bold text-slate-100 hover:bg-slate-900/50">My Orders</Link>
                    <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-sm font-bold text-rose-300 hover:bg-slate-900/50">Logout</button>
                  </div>
                )}
              </div>
            ) : (
              <div className="flex items-center gap-3">
                <Link to="/login" className="text-sm font-bold text-slate-100 hover:text-white/90 transition">Login</Link>
                <Link
                  to="/register"
                  className="inline-flex items-center rounded-full bg-linear-to-r from-cyan-400 to-sky-500 px-4 py-2 text-sm font-bold text-slate-950 shadow-sm shadow-cyan-400/40 hover:opacity-90 transition"
                >
                  Sign Up
                </Link>
              </div>
      ...
      {isMobileOpen && (
        <div className="md:hidden bg-slate-950/95 backdrop-blur-xl border-b border-slate-900/40">
          <div className="space-y-1 px-4 pt-2 pb-3">
            ...
            {token ? (
              <>
                <Link to="/orders" onClick={() => setIsMobileOpen(false)} className="block rounded-md px-3 py-2 text-base font-bold text-slate-100 hover:bg-slate-900/50">My Orders</Link>
                <button onClick={handleLogout} className="block w-full text-left rounded-md px-3 py-2 text-base font-bold text-rose-300 hover:bg-slate-900/50">Logout</button>
              </>
            ) : (
              <>
                <Link to="/login" onClick={() => setIsMobileOpen(false)} className="block rounded-md px-3 py-2 text-base font-bold text-slate-100 hover:bg-slate-900/50">Login</Link>
                <Link to="/register" onClick={() => setIsMobileOpen(false)} className="block rounded-md px-3 py-2 text-base font-bold text-slate-950 bg-linear-to-r from-cyan-400 to-sky-500 hover:opacity-90">Sign Up</Link>
              </>
            )}
          </div>
        </div>
